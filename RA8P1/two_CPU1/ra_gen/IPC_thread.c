...
#include "IPC_thread.h"
                ...
                static uint8_t IPC_thread_stack[40960] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
                static uint8_t IPC_thread_stack[40960] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.IPC_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #endif
                #endif
...
#endif
			IPC_thread_func, (const char*) "IPC_thread", 40960 / 4, // In words, not bytes
			(void*) &IPC_thread_parameters, //pvParameters
			1,
#if 0
                        (StackType_t *)&IPC_thread_stack,
                        (StaticTask_t *)&IPC_thread_memory
                        #else
			&IPC_thread
#endif
			);
